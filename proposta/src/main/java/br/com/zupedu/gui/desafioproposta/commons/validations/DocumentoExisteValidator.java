...
public class DocumentoExisteValidator implements ConstraintValidator<DocumentoExiste, String> {
    ...
    public boolean isValid(String s, ConstraintValidatorContext constraintValidatorContext) {
        if (s != null && repository.findByDocumentoHash(Encriptador.hashEncode(s.replaceAll("[^a-zA-Z0-9]", ""))).isPresent()) {
            throw new ApiBussinesException("Documento","Documento Ja esta atrelado a uma proposta", HttpStatus.UNPROCESSABLE_ENTITY);
        }
        return true;
    }
}
